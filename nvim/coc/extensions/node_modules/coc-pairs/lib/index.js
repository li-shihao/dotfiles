...
function activate(context) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        function insertPair(character, pair) {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                if (samePair && rest[0] == character && rest[1] != character) {
                    // move position
                    yield nvim.eval(`feedkeys("\\<Right>", 'int')`);
                    return '';
                }
        ...
        subscriptions.push(coc_nvim_1.workspace.registerKeymap(['i'], 'pairs-backspce', onBackspace));
        // tslint:disable-next-line: no-floating-promises
        nvim.resumeNotification(false, true);
        function createBufferKeymap(bufnr) {
            return tslib_1.__awaiter(this, void 0, void 0, function* () {
                let buf = nvim.createBuffer(bufnr);
                let pairs = (yield buf.getVar('coc_pairs'));
                if (!pairs)
                    return;
                ...
                for (let p of pairs) {
                    if (Array.isArray(p) && p.length == 2) {
                        ...
                        subscriptions.push(coc_nvim_1.workspace.registerExprKeymap('i', character, insertPair.bind(null, character, matched), false));
                        if (matched != character) {
                            subscriptions.push(coc_nvim_1.workspace.registerExprKeymap('i', matched, closePair.bind(null, matched), false));
                        }
                    }
        ...
        yield createBufferKeymap(coc_nvim_1.workspace.bufnr);
        coc_nvim_1.workspace.onDidOpenTextDocument(() => tslib_1.__awaiter(this, void 0, void 0, function* () {
            yield createBufferKeymap(coc_nvim_1.workspace.bufnr);
        }));
    });
}
            ...
            if (col > 1 && !/string/i.test(synname)) {
                ...
                if (col - 1 < buf.length) {
                    ...
                    if (pairs.has(pre) && pairs.get(pre) == next) {
                        yield nvim.eval(`feedkeys("\\<esc>ca${pre == '"' ? '\\"' : pre}", 'int')`);
                        return;
                    }
                }
            }
