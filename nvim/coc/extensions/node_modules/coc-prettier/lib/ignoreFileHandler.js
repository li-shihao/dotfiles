...
function ignoreFileHandler(disposables) {
    ...
    function getIgnorerForFile(fsPath) {
        const absolutePath = getIgnorePathForFile(fsPath, utils_1.getConfig(vscode_uri_1.default.file(fsPath)).ignorePath);
        if (!absolutePath) {
            return { ignoreFilePath: '', ignorer: nullIgnorer };
        }
        if (!ignorers.has(absolutePath)) {
            loadIgnorer(vscode_uri_1.default.file(absolutePath));
        }
        if (!fs_1.existsSync(absolutePath)) {
            // Don't log default value.
            const ignorePath = utils_1.getConfig(vscode_uri_1.default.file(fsPath)).ignorePath;
...
function getIgnorePathForFile(_filePath, ignorePath) {
    ...
    if (coc_nvim_1.workspace.workspaceFolder) {
        ...
        return folder ? getPath(ignorePath, vscode_uri_1.default.parse(folder.uri).fsPath) : null;
    }
    return null;
}
